...
import { SubCategory } from "@prisma/client"; // Giả sử bạn đang sử dụng Prisma

const API_URL = "/api/sub-category"; // URL của API

// Định nghĩa các action cho subCategory
const subCategoryAction = {
  ...
  // Tạo mới danh mục
  create: async (name: string): Promise<ApiResponse<SubCategory>> => {
    try {
  ...
  update: async (id: string, name: string): Promise<ApiResponse<SubCategory>> => {
    try {
  ...
  getByCategoryId: async (id: string): Promise<ApiResponse<SubCategory[]>> => {
    try {
      const response = await fetch(`${API_URL}/category/${id}`, { method: "GET" });
